---
title: "Cross-Disease Modeling - SFG PLS-DA"
...
# Searches for the keyword superior frontal gyrus within the human data set to reduce regional-dependent variation
Xhumandat <- Xhumandat[Xhumandat$Brain.region %in% "superior frontal gyrus",]
...
#svg('human_heatmap_filt_SFG.svg', width = 4, height = 4)
pheatmap(
...
plsda_draft_AD <- plsda(X_AD, Y_AD, ncomp = 4)
...
#svg('PLSDA-human_SFG.svg', width = 4, height = 4)
...
selected_words <- c("CSTA", "RCN3", "RPL36A", "RIN3")
...
loadingsAD$Gene <- factor(loadingsAD$Gene,
                  levels = c("RIN3", "RPL36A", "RCN3", "CSTA"))
...
#svg('PLSDA-LV1_loadings_SFG.svg', width = 4, height = 4)
...
loadingsAD %>%
        ggplot(aes(x = Gene, y = comp1, fill = Gene)) +
            ...
            scale_fill_manual(values = c("red3", "royalblue3", "royalblue3", "red3")) + 
...
# Specify the desired words in the desired order
selected_words <- c("RPL36A", "RCN3", "RIN3", "CSTA")
...
# Preparing factor to establish the barplot positioning in the figure across all groups
loadingsAD$Gene <- factor(loadingsAD$Gene,
                  levels = c("CSTA", "RIN3", "RCN3", "RPL36A"))

# Plot loadings with each variable identified with highest contribution to a specific class
plotLoadings(plsda_AD, comp = 2, method = 'mean', contrib = 'max')
#svg('PLSDA-LV2_loadings_SFG.svg', width = 4, height = 4)
# Plot of loadings color adjusted with fixed cytokine list
loadingsAD %>%
        ggplot(aes(x = Gene, y = comp2, fill = Gene)) +
            geom_bar(stat = "identity", color="black", show.legend = FALSE, width = 1) +
            scale_fill_manual(values = c("red3", "red3", "royalblue3", "royalblue3")) + 
